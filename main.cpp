...

{
    ...
    std::cout << p3.find_degree_of() << std::endl;
...
void read_txt(std::string input_file, std::string expected_file) {
    ...
    std::cout << "Expected degree: " << expected_poly.find_degree_of() << std::endl;
...
int main()
{
    ...
    // for (int i = 0; i <= 999; ++i) {
    //     terms.emplace_back(i, 1);
    // }

    // polynomial P1(terms.begin(), terms.end());
    ...
    // auto cf = P3.canonical_form();
    // for (auto [p, c] : cf) {
    //     if (p > 1998 || c <= 0) {
    //         std::cout << "ERROR: Term x^" << p << " has bad coefficient: " << c << "\n";
    //     }
    // }
